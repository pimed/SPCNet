...
def evaluate_per_pixel(mode, path_dict, output_filepath, folds, lr, bx_lesions=False, save_preds=True):
    ...
    if save_preds == True:
        ...
        for idx, case in enumerate(case_ids):
            ...
            export_volume(output_filepath, volume, avg_prediction_normal,
                          os.path.join(subdirectory, f'{case}_avg_norm_pred.nii'))

            avg_prediction_ind = np.mean(all_predictions_ind, -1)
            export_volume(output_filepath, volume, avg_prediction_ind,
                          os.path.join(subdirectory, f'{case}_avg_ind_pred.nii'))

            avg_prediction_agg = np.mean(all_predictions_agg, -1)
            export_volume(output_filepath, volume, avg_prediction_agg,
                          os.path.join(subdirectory, f'{case}_avg_agg_pred.nii'))
    ...
    for idx, case in enumerate(case_ids):
        ...
        if mode == 'model':
            ...
            for i in range(folds):
                ...
                pred = SPCNet().model_prediction(models[i], x)
        ...
        mod_prediction = SPCNet().return_mod_prediction(mask_np, pred_normal, pred_agg, pred_ind)
    ...
    stats_normal = evaluate_classifier(total_true_normal, total_pred_normal, [SPCNet().normal_threshold])  # ,
    # pred_thresh=total_pred_normal_thresh)
    total_stats_normal = pd.DataFrame.from_dict(stats_normal)

    stats_ind = evaluate_classifier(total_true_ind, total_pred_ind, [SPCNet().ind_threshold])  # ,
    # pred_thresh=total_pred_ind_thresh)
    total_stats_ind = pd.DataFrame.from_dict(stats_ind)

    stats_agg = evaluate_classifier(total_true_agg, total_pred_agg, [SPCNet().agg_threshold])  # ,
