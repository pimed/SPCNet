...
from utils.util_functions import prepare_data, evaluate_classifier, lesion_classifier, grade_lesion_classifier, generate_lesions
...
def evaluate_per_lesion(mode, path_dict, output_filepath, folds, lr, bx_lesions=False):
    ...
    # prepare data 
    t2_list, adc_list, mask_list, label_list, volume_list, case_ids = prepare_data(path_dict, cancer_only=False, bx_lesions=bx_lesions)
    ...
    for volume_thresh in volume_thresholds:
        ...
        stats = evaluate_classifier(total_model_true, total_model_pred,
                                         [1. - SPCNet_all().normal_threshold])  # , pred_thresh=total_model_pred_thresh)
        ...
        stats = evaluate_classifier(total_model_true_grade, total_model_pred_grade,
                                         [SPCNet_all().agg_threshold])  # , pred_thresh=total_model_pred_grade_thresh)
        total_stats = pd.DataFrame.from_dict(stats)
