...
def categorical_cross_entropy_balanced(num_normal=1, num_ind=1, num_agg=1):
    def loss(y_true, y_pred):
        ...
        y_pred /= K.sum(y_pred, axis=-1,
                        keepdims=True)  # ensure that entries for each pixel add to 1, should happen from softmax anyway
        ...
        cost = tf.losses.softmax_cross_entropy(onehot_labels=y_true, logits=K.log(y_pred), weights=weights,
                                               reduction='none')
